...
#   nation = "kinh"
#   nationality = "Việt Nam"
#   identification_number = "#{Faker::Code.asin}#{x+25000}"
#   religion = "không"
#   birthday = Faker::Date.between(20.years.ago, 18.years.ago)
#   User.create! name: name, email: email, password: pass, year: DateTime.now.year,
#     password_confirmation: pass_conf, role: role, address: address, phone: phone,
...
# users = User.student.limit(2750)
Register.unscoped.delete_all
users = User.student.limit(20)
users.each do |u|
  department_ids = u.find_user_departments
  major_ids = nndn.major_departments.get_by_depart(department_ids).pluck :major_id
  1.upto(3) do |n|
    major_id = major_ids.sample
    next if major_id.blank?
    major_ids.delete major_id
...
# users = User.student.where.not(id: users_done).limit(5000)
users = User.student.where.not(id: users_done).limit(30)
users.each do |u|
  department_ids = u.find_user_departments
  major_ids = spdn.major_departments.get_by_depart(department_ids).pluck :major_id
  1.upto(3) do |n|
    major_id = major_ids.sample
    next if major_id.blank?
    major_ids.delete major_id
...
users.each do |u|
  department_ids = u.find_user_departments
  major_ids = ktdn.major_departments.get_by_depart(department_ids).pluck :major_id
  1.upto(3) do |n|
    major_id = major_ids.sample
    next if major_id.blank?
    major_ids.delete major_id
